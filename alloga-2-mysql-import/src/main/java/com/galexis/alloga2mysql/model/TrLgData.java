package com.galexis.alloga2mysql.model;

// Generated Mar 4, 2013 4:41:13 PM by Hibernate Tools 4.0.0

import java.util.Date;

import javax.persistence.AttributeOverride;
import javax.persistence.AttributeOverrides;
import javax.persistence.Column;
import javax.persistence.EmbeddedId;
import javax.persistence.Entity;
import javax.persistence.Table;
import javax.persistence.Temporal;
import javax.persistence.TemporalType;

/**
 * TrLgData generated by hbm2java
 */
@Entity
@Table(name = "TR_LG_DATA")
public class TrLgData implements java.io.Serializable {

	private TrLgDataId id;

	public TrLgData() {
	}

	public TrLgData(TrLgDataId id) {
		this.id = id;
	}

	@EmbeddedId
	@AttributeOverrides({ @AttributeOverride(name = "crTime", column = @Column(name = "CR_TIME", nullable = false, length = 19)),
	        @AttributeOverride(name = "appId", column = @Column(name = "APP_ID", nullable = false, length = 10)),
	        @AttributeOverride(name = "tag", column = @Column(name = "TAG", nullable = false, length = 80)),
	        @AttributeOverride(name = "taskId", column = @Column(name = "TASK_ID", nullable = false, length = 36)),
	        @AttributeOverride(name = "transId", column = @Column(name = "TRANS_ID", nullable = false, length = 36)),
	        @AttributeOverride(name = "severity", column = @Column(name = "SEVERITY", nullable = false, length = 1)),
	        @AttributeOverride(name = "databin", column = @Column(name = "DATABIN", nullable = false)),
	        @AttributeOverride(name = "segmNo", column = @Column(name = "SEGM_NO", nullable = false, precision = 5, scale = 0)),
	        @AttributeOverride(name = "summary", column = @Column(name = "SUMMARY", nullable = false, length = 250)),
	        @AttributeOverride(name = "messagetyp", column = @Column(name = "MESSAGETYP", nullable = false, length = 50)) })
	public TrLgDataId getId() {
		return this.id;
	}

	public void setId(TrLgDataId id) {
		this.id = id;
	}

	@Temporal(TemporalType.TIMESTAMP)
	@Column(name = "CR_TIME", length = 19, updatable = false, insertable = false)
	public Date getRecordcreationtime() {
		return this.id.getCrTime();
	}

	public void setRecordcreationtime(Date recordcreationtime) {
		this.id.setCrTime(recordcreationtime);
	}

}
